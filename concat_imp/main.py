...
parser.add_argument('--dataset', default='trivago_imp', help='dataset name: diginetica/yoochoose1_4/yoochoose1_64/sample')
...
train_data = pickle.load(open(opt.dataset + '/train.pkl', 'rb'))
test_data = pickle.load(open(opt.dataset + '/test.pkl', 'rb'))
...
if opt.dataset == 'diginetica':
    ...
    n_node = 37484
elif opt.dataset == 'trivago_imp':
    n_node = 28596 #54904, half: 28595
    i_node = 260388 #401329, half: 260388
    embedding_len = 25
else:
    n_node = 310
#%%
...
#%%
model = GGNN_Concat_EMB(hidden_size=opt.hiddenSize, out_size=opt.hiddenSize, batch_size=opt.batchSize, n_node=n_node,
                 lr=opt.lr, l2=opt.l2,  step=opt.step, decay=opt.lr_dc_step * len(train_data.inputs) / opt.batchSize, lr_dc=opt.lr_dc,
                 nonhybrid=opt.nonhybrid, embedding_len=embedding_len, i_node=i_node)
print(opt)
#%%
# =============================================================================
...
for epoch in range(5):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        adj_in, adj_out, alias, item, mask, targets, imps, alias_t = train_data.get_slice(i)
        _, loss, _ = model.run(fetches, targets, item, adj_in, adj_out, alias, mask, imps, alias_t)
        loss_.append(loss)
    loss = np.mean(loss_)
