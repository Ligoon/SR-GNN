...
class Data():
    def __init__(self, data, sub_graph=False, method='ggnn', sparse=False, shuffle=False):
        ...
        self.impressions = np.asarray(data[2])
    ...
    def generate_batch(self, batch_size):
        if self.shuffle:
            ...
            self.impressions = self.impressions[shuffled_arg]
    ...
    def get_slice(self, index):
        ...
        impressions: numpy array. shape = (batch_size, )
        alias_target: numpy array, shape = (batch_size, )
        """
        if 1:
            ...
            if self.method == 'ggnn':
                for u_input in self.inputs[index]:
                    ...
                    # for add impression structure
                    alias_target = []
                    for t, i in zip(self.targets[index], self.impressions[index]):
                        idx = np.where(i==t)[0]
                        if(len(idx)==0):
                            alias_target.append(0)
                        else:
                            alias_target.append(idx[0])
                    alias_target = np.array(alias_target)
                return A_in, A_out, alias_inputs, items, self.mask[index], self.targets[index], self.impressions[index], alias_target
                ...
                for u_input in self.inputs[index]:
                    ...
                    alias_inputs.append([np.where(node == i)[0][0] for i in u_input])
                return A_in, A_out, alias_inputs, items, self.mask[index], self.targets[index], self.impressions[index]

        else:
            return self.inputs[index], self.mask[index], self.targets[index], self.impressions[index]